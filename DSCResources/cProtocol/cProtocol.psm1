...
function Get-TargetResource
{
    ...
    if(($sItem | Get-ItemProperty).Enabled -eq 4294967295 -and ($sItem | Get-ItemProperty).DisabledByDefault -eq 0)
    {
        $Ensure = "Present" 
    }
    else
    ...
    if(($cItem | Get-ItemProperty).Enabled -eq ($sItem | Get-ItemProperty).Enabled -and ($cItem | Get-ItemProperty).DisabledByDefault -eq ($sItem | Get-ItemProperty).DisabledByDefault)
    {
         $clientside = $true
    }
    else
